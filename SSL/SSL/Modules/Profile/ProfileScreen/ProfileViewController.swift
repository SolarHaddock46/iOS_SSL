...
final class ProfileViewController: UIViewController {
   ...
   private func createLogoutButton() -> PrimaryButtonContainer {
       let buttonContainer = PrimaryButtonContainer(localizationKey: "Log out")
   ...
   private func createTextField(withPlaceholder placeholder: String, isSecure: Bool) -> UserInfoTextFieldContainer {
       let textFieldContainer = UserInfoTextFieldContainer(placeholder: placeholder, isSecure: isSecure)
       return textFieldContainer
   }

   private func createButton(withText text: String) -> PrimaryButtonContainer {
       let buttonContainer = PrimaryButtonContainer(localizationKey: text)
       buttonContainer.addTarget(self, action: #selector(buttonTapped(_:)), for: .touchUpInside)
       return buttonContainer
   }

   @objc private func buttonTapped(_ sender: PrimaryButtonContainer) {
       guard let button = sender.subviews.first as? PrimaryButton else { return }
       
       print("Button tapped: \(button.titleLabel?.text ?? "")") // Debug print statement
   ...
   private func createView(for element: ProfileCardContentElement) -> UIView {
       ...
       case .textField(let placeholder, let isSecure):
           return createTextField(withPlaceholder: placeholder, isSecure: isSecure)
       case .button(let text):
           return createButton(withText: text)
       }
   }
   ...
   private func navigateToNewEmailSection() {
       ...
       snapshot.appendItems([
           .textFieldCard(elements: [
               .textField(placeholder: "New Email", isSecure: false),
               .button(text: "Save New Email")
           ])
       ], toSection: .newEmail)
   ...
   private func navigateToConfirmationCodeSection() {
       ...
       snapshot.appendItems([
           .textFieldCard(elements: [
               .textField(placeholder: "Confirmation Code", isSecure: false),
               .button(text: "Confirm")
           ])
   ...
   private func navigateToOldPasswordSection() {
       ...
       snapshot.appendItems([
           .textFieldCard(elements: [
               .textField(placeholder: "Old Password", isSecure: true),
   ...
   private func navigateToNewPasswordSection() {
       ...
       snapshot.appendItems([
           .textFieldCard(elements: [
               .textField(placeholder: "New Password", isSecure: true),
               .spacing(height: 16),
               .textField(placeholder: "Confirm New Password", isSecure: true),
   ...
   private func navigateToNameAndTelegramSection() {
       ...
       snapshot.appendItems([
           .textFieldCard(elements: [
               .textField(placeholder: "First Name", isSecure: false),
               .spacing(height: 16),
               .textField(placeholder: "Middle Name", isSecure: false),
               .spacing(height: 16),
               .textField(placeholder: "Last Name", isSecure: false),
               .spacing(height: 16),
               .textField(placeholder: "Telegram Username", isSecure: false),
               .button(text: "Save")
           ])
